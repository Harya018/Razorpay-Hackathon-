...
| **Security / robustness** | Strong, but with one open critical finding | Two independent red-team suites (`redteam/`, `red-team-agent/`) plus the Phase 17 trust-boundary suite were run live against the system, not claimed from code review. Five real gaps were found and fixed pre-submission (see `WHAT_BROKE.md`). **One critical finding remains open, not silently omitted:** Policy Gate's `/evaluate` currently trusts a caller-supplied price with no independent re-validation — a live, demonstrated exploit chain, documented in `WHAT_BROKE.md` §9, reported rather than hidden. |
